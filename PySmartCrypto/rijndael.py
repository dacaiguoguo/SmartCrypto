...
class rijndael:
    ...
    def encrypt(self, plaintext):
        if len(plaintext) != self.block_size:
            raise ValueError(f'wrong block length, expected {self.block_size} got {len(plaintext)}')
        Ke = self.Ke
    ...
    def decrypt(self, ciphertext):
        if len(ciphertext) != self.block_size:
            raise ValueError(f'wrong block length, expected {self.block_size} got {len(ciphertext)}')
        Kd = self.Kd
