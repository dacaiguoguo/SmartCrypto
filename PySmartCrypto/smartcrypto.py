...
def getFullUrl(urlPath):
    global tvIP, tvPort
    return f"http://{tvIP}:{tvPort}{urlPath}"

def GetFullRequestUri(step, appId, deviceId):
    return getFullUrl(f"/ws/pairing?step={step}&app_id={appId}&device_id={deviceId}")
...
def FirstStepOfPairing():
    ...
    firstStepURL = f"{GetFullRequestUri(0, AppId, deviceId)}&type=1"
...
def HelloExchange(pin):
    ...
    secondStepResponse = requests.post(secondStepURL, json=content).text
    print(f'secondStepResponse: {secondStepResponse}')
...
def AcknowledgeExchange(SKPrime):
    ...
    thirdStepResponse = requests.post(thirdStepURL, json=content).text
